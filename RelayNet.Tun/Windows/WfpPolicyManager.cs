...
namespace RelayNet.Tun.Windows
{
    ...
    internal sealed class WfpPolicyManager
    {
        ...
        public WfpPolicyManager(TunConfig config) => _config = config ?? throw new ArgumentNullException(nameof(config));
        ...
        private static IntPtr OpenEngine()
        {
            var session = new WfpNative.FWPM_SESSION0
            {
                ...
                displayData = new WfpNative.FWPM_DISPLAY_DATA0 { name = "RelayNet WFP Session", description = "RelayNet outbound enforcement session" },
                flags = 0u,
            ...
            int status = WfpNative.FwpmEngineOpen0(null!, WfpNative.RPC_C_AUTHN_WINNT, IntPtr.Zero, ref session, out IntPtr engine);
            if (status != WfpNative.ERROR_SUCCESS)
                throw new Win32Exception(status, "FwpmEngineOpen0 failed.");
            return engine;
        }
        ...
        private static void EnsureProvider(IntPtr engine)
        {
            var provider = new WfpNative.FWPM_PROVIDER0
            {
                ...
                displayData = new WfpNative.FWPM_DISPLAY_DATA0 { name = "RelayNet Provider", description = "RelayNet WFP provider" },
                flags = 0u,
            ...
            if (status != WfpNative.ERROR_SUCCESS && status != unchecked((int)0x80320009))
                throw new Win32Exception(status, "FwpmProviderAdd0 failed.");
        }
        ...
        private static void EnsureSublayer(IntPtr engine)
        {
            var sub = new WfpNative.FWPM_SUBLAYER0
            {
                ...
                displayData = new WfpNative.FWPM_DISPLAY_DATA0 { name = "RelayNet Sublayer", description = "RelayNet outbound sublayer" },
                flags = 0u,
                providerKey = ProviderKey,
            ...
            if (status != WfpNative.ERROR_SUCCESS && status != unchecked((int)0x8032000A))
                throw new Win32Exception(status, "FwpmSubLayerAdd0 failed.");
        }
        ...
        private static void InstallBootstrapAllowFilters(IntPtr engine, WfpBootstrapContext context, List<ulong> createdFilterIds)
        {
            ...
            {
                if (!IPAddress.TryParse(relayIp, out IPAddress? ip))
                    continue;
                ...
                try
                {
                    ...
                    var filter = new WfpNative.FWPM_FILTER0
                    {
                        ...
                        displayData = new WfpNative.FWPM_DISPLAY_DATA0 { name = $"RelayNet Bootstrap Allow {relayIp}", description = "Allow relay/control endpoint before full outbound block" },
                        flags = 0u,
        ...
        private static void RemoveSublayer(IntPtr engine)
        {
            ...
            if (status != WfpNative.ERROR_SUCCESS && status != unchecked((int)0x80320003))
                throw new Win32Exception(status, "FwpmSubLayerDeleteByKey0 failed.");
        }
        ...
        private static void RemoveProvider(IntPtr engine)
        {
            ...
            if (status != WfpNative.ERROR_SUCCESS && status != unchecked((int)0x80320003))
                throw new Win32Exception(status, "FwpmProviderDeleteByKey0 failed.");
        }

        private static int ResolveWintunInterfaceIndex(string adapterName)
        {
            NetworkInterface? nic = NetworkInterface.GetAllNetworkInterfaces().FirstOrDefault(n => string.Equals(n.Name, adapterName, StringComparison.OrdinalIgnoreCase));
            if (nic is null)
        ...
        private static void SaveManagedFilterIds(IEnumerable<ulong> ids)
        {
            ...
            File.WriteAllLines(FilterStatePath, ids.Select(x => x.ToString()));
        }
        ...
        private static void AbortTransactionBestEffort(IntPtr engine) => _ = WfpNative.FwpmTransactionAbort0(engine);
        private static void CloseEngineBestEffort(IntPtr engine) => _ = WfpNative.FwpmEngineClose0(engine);
    }
